...
function getMoves( $redis, $redis2, $row, $frc, $depth, $progress, $total ) {
	...
	$recurse = false;
	$dedup = ( $depth < 28 || ( ( xxhash64( $minbinfen ) & PHP_INT_MAX ) % 10 ) == 0 );
	if( isset($moves1['ply']) )
	{
...
*/
		...
		if( !$isloop )
		{
			...
			{
				$progress = $progress + log( count( $moves1 ) );
				$total = $total + log1p( exp( -$total ) ) + log( count( $moves1 ) );
				$updatemoves = array();
				//asort( $moves1 );
...
/*
						...
						if( $pid > 0 ) {
							...
							$redis->pconnect('dbserver.internal', 8888);
							$redis2->pconnect('dbserver.internal', 8890);
						}
						$total = 1;
*/
						$GLOBALS['curmove'] = $key;
					...
					$progress = $progress + log1p( exp( -$progress ) );
						...
						if( abs( $nextscore ) < 10000 ) {
							if( $nextcount > 1 )
								...
								if( count( $nextmoves ) > 1 ) {
									...
									$nextscore = ( int )round_stochastic( $nextscore * ( 0.9 + 0.1 * $t ), $minbinfen . $key . $nextscore );
								}
							}
						}
						if( $item != -$nextscore ) {
...
*/
		...
		if( count( $loopinfo ) > 0 ) {
			...
			if( $GLOBALS['counter'] % 10000 == 0 ) {
				gc_collect_cycles();
				echo implode(' ', array( $GLOBALS['counter'], number_format( exp( $progress - $total ) * 100, 2, '.', '' ) . "%", $GLOBALS['counter_dup'], number_format( $GLOBALS['counter_dup'] * 100 / $GLOBALS['counter'], 2, '.', '' ) . "%", $GLOBALS['counter_update'], number_format( $GLOBALS['counter_update'] * 100 / $GLOBALS['counter'], 2, '.', '' ) . "%", $GLOBALS['counter_ply'], number_format( $GLOBALS['counter_ply'] * 100 / $GLOBALS['counter'], 2, '.', '' ) . "%", $GLOBALS['curmove'], $depth, intval( ( $GLOBALS['counter'] + $GLOBALS['counter_dup'] - $GLOBALS['last_counter'] ) / ( time() - $GLOBALS['last_ts'] + 1 ) ), count( $GLOBALS['looptt'] ) ) ) . "\n";
/*
				$fp = fopen( 'chess_' . $GLOBALS['curmove'] . '.txt', 'w' );
				fwrite( $fp, implode("\t", array( $GLOBALS['counter'], number_format( exp( $progress - $total ) * 100, 2, '.', '' ) . "%", $GLOBALS['counter_dup'], number_format( $GLOBALS['counter_dup'] * 100 / $GLOBALS['counter'], 2, '.', '' ) . "%", $GLOBALS['counter_update'], number_format( $GLOBALS['counter_update'] * 100 / $GLOBALS['counter'], 2, '.', '' ) . "%", $GLOBALS['counter_ply'], number_format( $GLOBALS['counter_ply'] * 100 / $GLOBALS['counter'], 2, '.', '' ) . "%", $GLOBALS['curmove'], $depth, intval( ( $GLOBALS['counter'] + $GLOBALS['counter_dup'] - $GLOBALS['last_counter'] ) / ( time() - $GLOBALS['last_ts'] + 1 ) ), count( $GLOBALS['looptt'] ) ) ) . "\n" );
				fclose( $fp );
