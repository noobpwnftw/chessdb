...
include('/home/ssdb/api/php/SSDB.php');

$host = '/tmp/xiangqi.sock';
$ssdb = new SimpleSSDB($host);
...
function getAllScores( $redis, $minbinfen, $minindex, $hasLRmirror, $doc = null ) {
	$moves = array();
	$finals = array();
	if( $doc === null ) {
		...
		if( $minindex == 0 ) {
			foreach( $doc as $key => $item ) {
				...
				if( $key == 'a0a0' )
					...
					$minmove = min( $key, ccbgetLRmove( $key ) );
					if( abs( $item ) >= 30000 ) {
						if ( $item == -30001 ) {
							$item = 0;
						}
			...
			foreach( $doc as $key => $item ) {
				...
				if( $key == 'a0a0' )
					...
					$minmove = min( ccbgetBWmove( $key ), ccbgetLRBWmove( $key ) );
					if( abs( $item ) >= 30000 ) {
						if ( $item == -30001 ) {
							$item = 0;
						}
...
function updateScore( $redis, $minbinfen, $minindex, $hasLRmirror, $updatemoves ) {
	if( $hasLRmirror ) {
		...
		if( $minindex == 0 ) {
			$newmoves = array();
			foreach( $updatemoves as $key => $newscore ) {
				$newmoves[min( $key, ccbgetLRmove( $key ) )] = $newscore;
			}
			$redis->hMSet( $minbinfen, $newmoves );
		}
		else if( $minindex == 1 ) {
			$newmoves = array();
			foreach( $updatemoves as $key => $newscore ) {
				$newmoves[min( ccbgetBWmove( $key ), ccbgetLRBWmove( $key ) )] = $newscore;
			}
			$redis->hMSet( $minbinfen, $newmoves );
		}
	}
...
function getMoves( $ssdb, $redis, $row, $recurse, $doc = null, $doc_key = null ) {
	...
	if( $recurse && $minbinfen !== $doc_key ) {
		echo 'error: ' . $row . "\n";
		$ssdb->hclear( $doc_key );
		return;
	}

	list( $moves1, $finals ) = getAllScores( $redis, $minbinfen, $minindex, $hasLRmirror, $doc );
	...
	if( $recurse ) {
		...
		$moves1 = array_diff_key( ccbmovegen( $row ), $moves1 );
		if( count( $moves1 ) > 0 ) {
			$nextarr = array();
			$nextprobe = array();
			foreach( $moves1 as $key => $item ) {
				$nextfen = ccbmovemake( $row, $key );
				$nextLRfen = ccbgetLRfen( $nextfen );
				$nextBWfen = ccbgetBWfen( $nextfen );
				$nexthasLRmirror = ( $nextfen == $nextLRfen ? false : true );
				if( $nexthasLRmirror ) {
					$nextLRBWfen = ccbgetLRfen( $nextBWfen );
					list( $nextminbinfen, $nextminindex ) = getBinFenStorage( array( ccbfen2hexfen($nextfen), ccbfen2hexfen($nextBWfen), ccbfen2hexfen($nextLRfen), ccbfen2hexfen($nextLRBWfen) ) );
				}
				else {
					list( $nextminbinfen, $nextminindex ) = getBinFenStorage( array( ccbfen2hexfen($nextfen), ccbfen2hexfen($nextBWfen) ) );
				}
				$nextarr[$nextminbinfen] = $key;
				if( !isset( $GLOBALS['cached'][xxhash64( $nextminbinfen ) & PHP_INT_MAX] ) ) {
					$nextprobe[] = $nextminbinfen;
				}
			}
			$nextsizes = array();
			if( count( $nextprobe ) > 0 )
				$nextsizes = $ssdb->multi_hsize( $nextprobe );
			$nextknown = array();
			foreach( $nextsizes as $next => $item ) {
				if( (int)$item <= 0 ) {
					$GLOBALS['cached'][xxhash64( $next ) & PHP_INT_MAX] = 1;
					continue;
				}
				$nextknown[] = $next;
			}
			$updatemoves = array();
			foreach( $nextknown as $next ) {
				$key = $nextarr[$next];
				$nextfen = ccbmovemake( $row, $key );
				$nextmoves = getMoves( $ssdb, $redis, $nextfen, false );
...
while(1) {
	...
	if($pid > 0) {
		...
		$GLOBALS['cached'] = new Judy( Judy::BITSET );
		$ssdb = new SimpleSSDB($host);
		$redis = new Redis();
		$redis->connect($host);
		foreach($entries as $entry) {
			$name = $entry[0];
			$row = ccbhexfen2fen(bin2hex($name));
			getMoves($ssdb, $redis, $row, true, $entry[1], $name);
		}
		exit(0);
	}
}
