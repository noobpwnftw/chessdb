...
function getMoves( $redis, $redis2, $row, $depth ) {
	...
	$recurse = false;
	//$dedup = false;
	$dedup = ( ( $depth <= 10 || $depth % 4 == 1 ) && count( $moves1 ) > 2 );
	$iswhite = ( substr( $row, strpos( $row, ' ' ) + 1, 1 ) == 'w' );
	if( isset( $moves1['ply'] ) )
	{
		$packedply = $moves1['ply'];
		$storedplyw = ( ( ( $packedply >> 8 ) & 0xFF ) << 1 ) - 2;
		$storedplyb = ( ( $packedply & 0xFF ) << 1 ) - 1;
		$storedply = ( $storedplyw >= 0 && $storedplyb > 0 ) ? min( $storedplyw, $storedplyb ) : max( $storedplyw, $storedplyb );
		if( $iswhite )
			$packedply = ( $packedply & 0x00FF ) | ( ( ( $depth >> 1 ) + 1 ) << 8 );
		else
			$packedply = ( $packedply & 0xFF00 ) | ( ( $depth + 1 ) >> 1 );
	}
	else
	{
		$storedply = -1;
		if( $iswhite )
			$packedply = ( ( $depth >> 1 ) + 1 ) << 8;
		else
			$packedply = ( $depth + 1 ) >> 1;
	}
	if( $storedply >= 0 )
	{
		$stmply = $iswhite ? $storedplyw : $storedplyb;
		if( $stmply < 0 || $stmply > $depth )
		{
			updatePly( $redis, $minbinfen, $packedply );
			if( $storedply > $depth )
				$dedup = false;
		}
	}
	...
	if( $storedply < 0 || $storedply >= $depth )
	{
	...
	if( $recurse && $depth < $GLOBALS['max_depth'] )
	{
/*
...
_respawn:
				...
				foreach( $moves1 as $key => $item ) {
					if( $depth == 0 ) {
						...
						if( $pid > 0 ) {
							...
							$redis->connect('/tmp/xiangqi.sock');
							$redis2->connect('/tmp/dedup2.sock');
						}
						$GLOBALS['curmove'] = $key;
					}
					if( isset( $finals[ $key ] ) )
				...
				if( $depth == 0 ) {
					if( $pid > 0 ) {
						...
						if( $reaped > 0 && isset( $childs[ $reaped ] ) ) {
							unset( $moves1[ $childs[ $reaped ] ] );
							unset( $childs[ $reaped ] );
						}
						if( count( $moves1 ) > 0 ) {
...
try{
	...
	$redis2 = new Redis();
	$redis2->connect('/tmp/dedup2.sock');
	$resume_depth = $redis2->get('max_depth');
	$resume_depth = ( $resume_depth !== FALSE ) ? max( 10, (int)$resume_depth ) : 10;
	for( $GLOBALS['max_depth'] = $resume_depth; $GLOBALS['max_depth'] <= 500; $GLOBALS['max_depth'] += 10 ) {
		$GLOBALS['counter'] = 0;
		...
		if( getmypid() != $pid )
			exit(0);
		echo 'max_depth=' . $GLOBALS['max_depth'] . ' ok' . "\n";
		$redis2->connect('/tmp/dedup2.sock');
		$redis2->rawCommand('flushdb');
		$redis2->set( 'max_depth', min( 500, $GLOBALS['max_depth'] + 10 ) );
		$redis2->close();
		$compact_pid = pcntl_fork();
		if( $compact_pid == 0 ) {
			$redis2->connect('/tmp/dedup2.sock', 0, 0, '', 0, -1);
			$redis2->rawCommand('compact', 'all');
			exit( 0 );
		}
		$redis2->connect('/tmp/dedup2.sock');
	}
