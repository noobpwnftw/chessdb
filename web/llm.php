...
try{
	...
	$querylimit = $memcache_obj->get( 'QLimit2::' . $_SERVER['REMOTE_ADDR'] );
	if( $querylimit === FALSE || $querylimit < 2 )
	{
		$memcache_obj->add( 'QLimit2::' . $_SERVER['REMOTE_ADDR'], 0, 0, 1 );
		$memcache_obj->increment( 'QLimit2::' . $_SERVER['REMOTE_ADDR'] );
			...
			if( isset( $game ) && isset( $moves_json ) ) {
				...
				if( isset( $parsed['status'] ) && $parsed['status'] == 'ok' ) {
					...
					if( $cached !== FALSE ) {
						...
						$note = $top['note'];
						$abs_score = abs( $score );
						if( strpos( $note, 'W' ) !== false ) $decisive = 'tablebase win for side-to-move';
						else if( strpos( $note, 'L' ) !== false ) $decisive = 'tablebase loss for side-to-move';
						else if( strpos( $note, 'D' ) !== false ) $decisive = 'tablebase draw';
						else if( $abs_score >= 10000 ) {
							$decisive = ( $score > 0 ) ? 'forced mate for side-to-move' : 'forced mate against side-to-move';
						}
						if( $abs_score < 50 ) $tier = 'roughly equal';
						else if( $abs_score < 150 ) $tier = ( $score >= 0 ? 'slight edge for side-to-move' : 'slight edge against side-to-move' );
						else $tier = ( $score >= 0 ? 'clear edge for side-to-move'  : 'clear edge against side-to-move' );
...
TIER_LABEL: text label describing evaluation strength (e.g., "roughly equal", "clear edge for side-to-move").
DECISIVE: optional override string (e.g., "tablebase win for side-to-move", "forced mate against side-to-move").
...
- Written in LANG.
- No lists, markdown, or code fences.
EOT;
