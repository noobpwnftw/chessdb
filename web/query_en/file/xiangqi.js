...
function PlaceDot(cid) {
	...
	if (desk[k[0]][k[1]] != 0) {
		elem.innerHTML = "<img alt='' onmousedown='mdown(\"" + cid + "\")' onstyle='position:absolute;z-index:7' src='/file/cap.gif'>";
	} else {
		elem.innerHTML = "<img alt='' onmousedown='mdown(\"" + cid + "\")' onstyle='position:absolute;z-index:7' src='/file/waypoint.gif'>";
	}
...
function AddFigure(x, y, id) {
	...
	elem.style.position = 'absolute';
	elem.innerHTML = "<img alt='' onmousedown='onmdown2(event,\"" + s + "\")' style='position:absolute;z-index:6' id='" + s + "'src='/file/" + id.toLowerCase() + ".png'>";
	plaza.appendChild(elem);
	plaza.lastChild.style.left = a;
	plaza.lastChild.style.top = b;
	plaza.lastChild.style.width = 40 + 'px';
