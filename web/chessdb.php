...
function count_attacker_pieces( $fen ) {
	...
	$count = 0;
	if( $color == 'b' ) {
		for( $i = 0, $limit = strlen( $board ); $i < $limit; $i++ ) {
			switch( $board[$i] ) {
			case 'r':
			case 'n':
			case 'c':
...
function scoreExists( $redis, $minbinfen, $minindex, $hasLRmirror, $move ) {
	if( $hasLRmirror ) {
		...
		if( $minindex == 0 ) {
			return $redis->hExists( $minbinfen, min( $move, ccbgetLRmove( $move ) ) );
		}
		else {
			return $redis->hExists( $minbinfen, min( ccbgetBWmove( $move ), ccbgetLRBWmove( $move ) ) );
		}
	}
}
...
function updateQueue( $row, $key, $priority ) {
	...
	if( $hasLRmirror ) {
		...
		list( $minbinfen, $minindex ) = getBinFenStorage( array( ccbfen2hexfen($row), ccbfen2hexfen($BWfen) ) );
		if( $minindex == 0 ) {
			$fq->push( 0, $minbinfen, min( $key, ccbgetLRmove( $key ) ), $priority, time() + 7200, true, true );
		}
		else if( $minindex == 1 ) {
			$fq->push( 0, $minbinfen, min( ccbgetBWmove( $key ), ccbgetLRBWmove( $key ) ), $priority, time() + 7200, true, true );
		}
	}
}
...
function getMoves( $redis, $row, $banmoves, $update, $mirror, $learn, $depth ) {
	...
	$iswhitelane = ( substr( $row, strpos( $row, ' ' ) + 1, 1 ) == 'w' ) != ( $minindex == 1 || $minindex == 3 );
	if( isset($moves1['ply']) )
	{
		$packedply = $moves1['ply'];
		if( $iswhitelane ) {
			$storedply = ( ( $packedply >> 8 ) & 0xFF ) << 1;
			$packedply = ( $packedply & 0x00FF ) | ( ( $depth >> 1 ) << 8 );
		}
		else {
			$storedply = ( ( $packedply & 0xFF ) << 1 ) - 1;
	...
	if( isset( $_REQUEST['board'] ) && !empty( $_REQUEST['board'] ) ) {
		...
		if( isset( $row ) && !empty( $row ) ) {
			...
			else
			{
				...
				if( $querylimit === FALSE || $querylimit <= $ratelimit || $_SERVER['SERVER_ADDR'] == $_SERVER['REMOTE_ADDR'] )
				{
					$egtbresult = NULL;
