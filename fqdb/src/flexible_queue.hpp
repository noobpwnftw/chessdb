...
namespace FlexibleQueue {
	...
	template <size_t CAP> class MPMCRing {
	...
	public:
		...
		explicit FlexibleQueueImpl(const Options& opt)
			...
			stop_(false), pending_work_(false), snapshot_child_pid_(0),
		...
		PushResult push(ByteSpan key, Pri priority, uint64_t expiry,
			...
			auto* op = new PushOp(key, effective, upsert, priority,
				expiry, value);
		...
		PushResult push(ByteSpan key, uint64_t expiry, ByteSpan value = {},
			...
			auto* op = new PushOp(key, effective, upsert, expiry, value);
			auto fut = op->done.get_future();
			enqueue_({ op, KIND_PUSH });
			return fut.get();
		}
		std::vector<QueueItem> pop(int N = 1, PopOrder order = PopOrder::EAsc) {
			auto* op = new PopOp{ std::promise<std::vector<QueueItem>>(), N, order };
			auto fut = op->done.get_future();
		...
		std::vector<QueueItem> refresh(uint64_t threshold, uint64_t expiry, int N = 1) {
			auto* op = new RefreshOp{ std::promise<std::vector<QueueItem>>(), threshold, expiry, N};
			auto fut = op->done.get_future();
			enqueue_({ op, KIND_REFRESH });
			return fut.get();
		}
		bool remove(ByteSpan key) {
			auto* op = new RemoveOp{ std::promise<bool>(), key };
			auto fut = op->done.get_future();
		...
		size_t count(int min_priority = -1, int max_priority = 65535) {
			...
			auto* op = new CountOp{ OpExtStorage<WithPriority>{ min_priority, max_priority }, std::promise<size_t>() };
			auto fut = op->done.get_future();
		...
		template <bool W = WithPriority, typename std::enable_if<!W, int>::type = 0>
		size_t count() {
			auto* op = new CountOp{ OpExtStorage<WithPriority>{}, std::promise<size_t>() };
			auto fut = op->done.get_future();
			enqueue_({ op, KIND_COUNT });
			return fut.get();
		}
		void snapshot_now() {
			auto* op = new SnapOp{ std::promise<void>() };
			auto fut = op->done.get_future();
		...
		void push_internal_(const Node& n, bool check = true, Node* current = nullptr) {
			...
			if (!current) {
				...
				if constexpr (WithPriority) {
					...
					if (it == items_.end()) {
						auto pr = items_.emplace(std::piecewise_construct,
							std::forward_as_tuple(n.priority),
			...
			while (true) {
				{
					...
					cv_.wait_for(lk, std::chrono::seconds(1), [this]{
						return stop_ || pending_work_;
					});
					if (stop_ && !pending_work_)
						break;
					pending_work_ = false;
				}
		...
		void spawn_snapshot_writer_(uint64_t now_sec) {
			if (snapshot_child_pid_ == 0) {
				...
				if (!snap_path_.empty()) {
					pid_t pid = ::fork();
					if (pid == 0) {
						write_snapshot_(dir_of_(snap_path_).c_str());
						::_exit(0);
					}
					else if (pid > 0) {
