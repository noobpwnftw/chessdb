...
var statsurl = new String(PREFIX + 'statsc.php?lang=1');
...
function PlaceDot(cid) {
	...
	if (desk[k[0]][k[1]] != 0) {
		elem.innerHTML = "<img alt='' onmousedown='mdown(\"" + cid + "\")' ondragover='ondover(event)' ondragenter='ondenter(event)' ondragleave='ondleave(event)' ondrop='ond(event, \"" + cid + "\")' onmouseenter='ondenter(event)' onmouseleave='ondleave(event)' style='position:absolute;z-index:7' src='/file/chess/cap.gif'>";
	} else {
		elem.innerHTML = "<img alt='' onmousedown='mdown(\"" + cid + "\")' ondragover='ondover(event)' ondragenter='ondenter(event)' ondragleave='ondleave(event)' ondrop='ond(event, \"" + cid + "\")' onmouseenter='ondenter(event)' onmouseleave='ondleave(event)' style='position:absolute;z-index:7' src='/file/chess/waypoint.gif'>";
	}
...
function GetInnerPage(e) {
	if(Vrulecheck.checked)
	{
		...
		{
			ClearInner();
			Vout.innerHTML += '<span style="text-align:center; display:block;">Draw by threefold repetition!<\/span>';
			SyncHistory();
		}
		else if(chess.insufficient_material())
		{
			ClearInner();
			Vout.innerHTML += '<span style="text-align:center; display:block;">Draw by insufficient material!<\/span>';
			SyncHistory();
		}
		else
		{
...
function GetMoveList(s) {
	...
	if (s.search(/unknown/) != -1) {
		ClearInner();
		Vout.innerHTML += '<span style="text-align:center; display:block;">This position is not yet analyzed, you can:<table style="margin-left: auto;margin-right: auto;"><tr><td onClick="RequestQueue()" class="button">&nbsp;Request for analysis&nbsp;<\/td><\/tr><\/table><\/span>';
		if(wb == 0 && Vwauto.checked) {
			alert('No AI move for white!');
		...
		ClearInner();
		if (wb == 0) {
			Vout.innerHTML += '<span style="text-align:center; display:block;">White is checkmated!<\/span>';
		}
		else {
		...
		ClearInner();
		if (wb == 0) {
			Vout.innerHTML += '<span style="text-align:center; display:block;">White is stalemated!<\/span>';
		}
		else {
	...
	if( !Vhidescore.checked ) {
		s = '<table cellspacing="0" style="text-align:center;" class="movelist"><thead><tr style="height:20px;"><td><b>Move<\/b><\/td><td><b>Rank<\/b><\/td><td><b>Score<\/b><\/td><td style="min-width:100px;padding-right:20px;"><b>Notes<\/b><\/td><\/tr><\/thead><tbody style="height:480px">';
		var skip = 0;
...
function ondenter(ev) {
	ev.target.parentElement.style.background = "rgba(180, 180, 180, 0.6)";
}
