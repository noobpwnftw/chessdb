...
void calc_egtb_index(Position& pos, char* names, S8& mirror, U64& offset);
...
static inline void log_err(const char* what, const char* ctx) {
#ifdef EGTB_LOGGING
    syslog(LOG_ERR | LOG_USER, "%s: %s", what, ctx);
#else
    (void)what; (void)ctx;
#endif
}
...
static inline int decode_root_dtc(U16 raw, U64 flags, int& out_order) {
    out_order = (raw >> 10) & 0x3f;
    int step;
    if (flags) { if (raw & 512) out_order += 64; step = raw & 0x1ff; }
    else       { step = raw & 0x3ff; }
    if (step == 0) return 0;
    const bool even = ((step & 1) == 0);
    if (out_order > 0) return even ? (20000 - step) : (step - 20000);
    return even ? (30000 - step) : (step - 30000);
}
static inline void decode_child_dtm(U16 raw, int& out_score, int& out_step) {
    const int flag = (raw >> 11) & 1;
    out_step = raw & 0x7ff;
    if (out_step == 0) { out_score = 0; return; }
    out_score = (flag != 0) ? (out_step - 30000) : (30000 - out_step);
}
static inline void decode_child_dtc(U16 raw, U64 flags, int& out_score, int& out_order, int& out_step) {
    out_order = (raw >> 10) & 0x3f;
    if (flags) { if (raw & 512) out_order += 64; out_step = raw & 0x1ff; }
    else       { out_step = raw & 0x3ff; }
    if (out_step == 0) { out_score = 0; return; }
    const bool even = ((out_step & 1) == 0);
    if (out_order > 0) out_score = even ? (out_step - 20000) : (20000 - out_step);
    else               out_score = even ? (out_step - 30000) : (30000 - out_step);
}
...
struct FileCtx {
    ...
    bool is_singular[2]{false,false};
    int  single_val[2]{0,0};
    U32  tail_size[2]{0,0};
    U32  block_size[2]{0,0};
    U32  block_cnt[2]{0,0};
    U64  data_size[2]{0,0};
    U64  data_start[2]{0,0};
    U64  hdr_table_off[2]{0,0};
...
struct BlockKeyHash {
    std::size_t operator()(const BlockKey& k) const noexcept {
        ...
        std::size_t h2 = std::hash<uint64_t>{}(static_cast<uint64_t>(k.blk_index) ^ (static_cast<uint64_t>(static_cast<uint8_t>(k.side)) << 56));
...
public:
    ...
    std::shared_ptr<FileCtx> get_file_ctx(const char* file_name) {
        ...
        if (!pread_exact(ctx->fd, ctrl, sizeof(ctrl), 0)) {
            log_err("EGTB read ctrl header failed", file_name);
            return nullptr;
        }
        const U32 key = ctrl[1];
        ctx->table_num = static_cast<U8>(key & 3);

        U64 off = sizeof(ctrl);
        for (int i = 0; i < ctx->table_num; ++i) {
            U8 head[2];
            if (!pread_exact(ctx->fd, head, sizeof(head), off)) {
                log_err("EGTB read table header failed", file_name);
                return nullptr;
            }
            off += sizeof(head);
            ...
            if (!ctx->is_singular[i]) {
                ...
                if (!pread_exact(ctx->fd, &hdr, sizeof(hdr), off)) {
                    log_err("EGTB read block header failed", file_name);
                    return nullptr;
                }
                                         ...
                                         S8 side, U64 blk_index)
    {
        ...
        if (probe) {
            ...
            const size_t comp_size = static_cast<size_t>(block_hdr & EGTB_LOW20_MASK);
            ...
            size_t decwant = decode_size;
            size_t srclen  = comp_size - 5;
            const char* props = src.data() + (comp_size - 5);
            int rc = uncompress_lzma(dst.data(), &decwant, src.data(), &srclen, props);
            {
                std::lock_guard<std::mutex> lk(slot->m);
                if (rc != SZ_OK || decwant != decode_size) {
                    slot->error = true;
                } else {
                    dst.resize(decwant);
                    slot->data.swap(dst);
                    slot->decoded_size = decwant;
                    slot->done = true;
                }
                slot->probing = false;
                slot->cv.notify_all();
...
PHP_FUNCTION(ccegtbprobe)
{
    ...
    std::vector<ProbeTask> tasks(static_cast<size_t>(task_count));
    ...
    for (int i = 0; i < task_count; ++i) {
        ...
        const bool no_heavy = (pos.number[WhitePawn] + pos.number[BlackPawn] +
    ...
    for (int i = 0; i < task_count; ++i) {
        ...
        if (!t.found) {
            ...
            pos.move_do(t.move);
            pos.to_fen(err_fen);
            log_err("Error probing EGTB", err_fen);
            pos.move_undo();
#endif
        }
    }
...
static bool read_egtb_value_cached(const char* file_name, S8 side, U64 offset, U16& out_score, U64& out_flags, ProbeSession* session)
{
    auto fctx = session->get_file_ctx(file_name);
    if (!fctx) return false;

    if (side == Black && fctx->table_num != 2) {
        log_err("EGTB table side not exist", file_name);
        return false;
    }

    if (fctx->is_singular[side]) {
        out_score = static_cast<U16>(fctx->single_val[side]);
        out_flags = static_cast<U64>(fctx->single_val[side]);
        return true;
    }

    out_flags = static_cast<U64>(fctx->single_val[side]);

    const U64 entries_per_block = static_cast<U64>(fctx->block_size[side] >> 1);
    const U64 blk_index = offset / entries_per_block;
    if (blk_index >= fctx->block_cnt[side]) return false;
    const U64 within_block = offset - (entries_per_block * blk_index);

    std::shared_ptr<BlockSlot> slot = session->get_block(fctx, side, blk_index);
    if (!slot || slot->error || !slot->done) return false;

    U16 val = 0;
    const size_t byte_index = static_cast<size_t>(within_block) * 2;
    if (byte_index + 2 > slot->data.size()) return false;
    std::memcpy(&val, slot->data.data() + byte_index, sizeof(U16));
    out_score = val;
    return true;
}
...
void calc_egtb_index(Position& pos, char* names, S8& mirror, U64& offset)
{
    ...
    int compress_id = 0;
    const Group_Info* info = nullptr;
    float best_ratio = 100.0f;
    for (int i = 0; i < 10; ++i)
    {
        if (set_id[i] == 0) continue;
        info = get_set_info(set_id[i]);
        const float ratio = static_cast<float>(info->compress_size) / info->table_size;
    ...
    if ((compress_ix & 0xffffU) < info->compress_size)
    {
        for (int i = 0; i < 10; ++i)
        {
            ...
            info = get_set_info(set_id[i]);
            if (i == compress_id) {
                offset += (static_cast<U64>(compress_ix & 0xffffU)) * weight;
                weight *= info->compress_size;
            } else {
                offset += (static_cast<U64>(info->get_list_pos(square_list[i]) & 0xffffU)) * weight;
    ...
    else
    {
        for (int i = 0; i < 10; ++i)
        {
            ...
            info = get_set_info(set_id[i]);
            if (i == compress_id) {
                offset += (static_cast<U64>((compress_ix >> 16) & 0xffffU)) * weight;
                weight *= info->compress_size;
            } else {
                const U32 v = info->get_list_pos(square_list[i]);
                offset += (static_cast<U64>((v >> 16) & 0xffffU)) * weight;
                weight *= info->table_size;
            }
        }
    }
