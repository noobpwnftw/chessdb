...
try{
	...
	if( $egtbstats !== FALSE ) {
		...
		if( $egtbstats !== FALSE ) {
			...
			if( is_array( $decoded_stats ) && count( $decoded_stats ) === 4 ) {
				list( $egtb_count_dtc, $egtb_size_dtc, $egtb_count_dtm, $egtb_size_dtm ) = $decoded_stats;
				$memcache_obj->set( 'EGTBStats', $decoded_stats, 0, 86400 );
			}
		}
	}
	$nps = 0;
	...
	$newrate = (int)max( 5, 1000 * pow( 1 + ( $queue_count_prio / 5 + $sel_count_prio + $queue_count / 50 + $sel_count / 10 ) / 200, -1 ) );
	$memcache_obj->set( 'RateLimit', $newrate );
	if( $isJson ) {
		header('Content-type: application/json');
		echo '{"status":"ok","positions":' . $pos_count . ',"queue":{"scoring":' . $queue_count . ',"sieving":' . $sel_count . '},"worker":{"backlog":' . (int)($est * 60) . ',"speed":' . (int)($nps * 1000) . '},"egtb":{"count":{"dtc":' . $egtb_count_dtc . ',"dtm":' . $egtb_count_dtm . '},"size":{"dtc":' . $egtb_size_dtc . ',"dtm":' . $egtb_size_dtm . '}}}';
	} else {
