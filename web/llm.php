					...
					if( $cached !== FALSE ) {
						...
						foreach( $parsed['moves'] as $move ) {
							if( count( $moves_eval ) > 4 )
								break;
							if( $game === 'CHESS' )
								$moves_eval[] = array( 'san' => $move['san'], 'score' => $move['score'] / 100 );
							else
								$moves_eval[] = array( 'uci' => $move['uci'], 'score' => $move['score'] / 100 );
						}
...
- Scores in MOVES_EVAL are centipawn evaluations (scale: 1.00 = one pawn's worth) oriented in the SIDE_TO_MOVE perspective.
...
EOT;
						...
						curl_setopt_array($ch, [
							...
							CURLOPT_TIMEOUT => 30,
							CURLOPT_WRITEFUNCTION => function ($ch, $chunk) use (&$buffer, &$final) {
								$buffer .= $chunk;
