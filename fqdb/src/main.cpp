...
alignas(4) int g_pending_work = 0;
...
static Queue<Job, MAX_CAP> g_queue;
...
static void worker_loop() {
	while (true) {
		{   // wait for work or stop
			std::unique_lock<std::mutex> lk(g_fx_mutex);
			while (!g_stop.load(std::memory_order_acquire) && g_pending_work == 0) {
				lk.unlock();
				futex_wait(&g_pending_work, 0);
				lk.lock();
			}
			if (g_stop.load(std::memory_order_acquire) && g_pending_work == 0)
				break;
			g_pending_work = 0;
		}
		Job job;
...
struct ConnThread {
	...
	void enqueue_new_conn(int fd) {
		{
			std::lock_guard<std::mutex> lk(nq_mu);
			new_conns.emplace_back(fd);
		}
		uint64_t one = 1; ::write(efd, &one, sizeof(one));
	}
		...
		while (true) {
			...
			if (cs.phase == ConnState::READ_PAYLOAD) {
				...
				bool need_wake;
				{
					...
					need_wake = (g_pending_work == 0);
					g_pending_work = 1;
				}
				if (need_wake)
					futex_wake(&g_pending_work, 1);
	...
	void mod_events(int fd, uint32_t flags) { struct epoll_event ev {}; ev.events = flags; ev.data.fd = fd; ::epoll_ctl(epfd, EPOLL_CTL_MOD, fd, &ev); }
...
int main(int argc, char** argv) {
	...
	futex_wake(&g_pending_work, INT_MAX);  // let workers leave wait()
